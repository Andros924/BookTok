...
import { Film, Calendar, User, CheckCircle, Clock, AlertTriangle, Plus } from 'lucide-react'
import movieLoanService from '../services/movieLoanService'

function MovieLoanList() {
  ...
  const fetchLoans = async () => {
    try {
      const loansData = await movieLoanService.getAllLoans()
      setLoans(loansData)
    } catch (error) {
  ...
  const handleReturnMovie = async (loanId) => {
    try {
      await movieLoanService.returnLoan(loanId)
    ...
    <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      <div className="px-4 py-6 sm:px-0">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Gestione Prestiti Film</h1>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-shrink-0">
                        {loan.movies?.cover_image ? (
                          <img
                            src={loan.movies.cover_image}
                            alt={loan.movies.title}
                      ...
                      {/* Informazioni film e prestito */}
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-gray-900">
                          {loan.movies?.title}
                        </h3>
                        <p className="text-gray-600">Diretto da {loan.movies?.director}</p>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      {loan.status === 'active' && (
                        <button
                          onClick={() => handleReturnMovie(loan.id)}
